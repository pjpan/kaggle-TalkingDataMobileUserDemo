...
cv <- rbinom(nrow(gender_age_train), 1, 0.8)==1
...
dtrain <- xgb.DMatrix(as.matrix(gender_age_train_cv1), label = gender_age_train_cv1$group, missing = NA)
dtest <- xgb.DMatrix(as.matrix(gender_age_train_cv2), label = gender_age_train_cv2$group, missing = NA)

set.seed(831)
watchlist = list(train = dtrain, eval = dtest)
xgb_cv <- xgb.cv(params = param,
                 data = dtrain,
                 # label = gender_age_train_cv1$group,
                 nrounds = 10,
                 maximize = F,
                 # watchlist = watchlist,
                 nfold = 5,
